...
static ssize_t AsyncIo(int fd, SysFunType sys_fun, const char* sys_fun_name,
    ...
    // 如果没有Hook
    if (!fish::IsHookEnable()) {
        return sys_fun(fd, std::forward<Args>(args)...);
    }

    // 取得套接字，如果取不到说明非网络套接字
    fish::FdCtx::Ptr ctx = fish::FdManagerSgt::Instance().Get(fd);
    if (!ctx) {
        return sys_fun(fd, std::forward<Args>(args)...);
    }
...
retry:
    ...
    if (n == -1 && errno == EAGAIN) {
        auto iom = fish::IoManager::GetCurIoManager();
        fish::Timer::Ptr timer;
        ...
        int rt = iom->SetEvent(fd, event, fish::Coroutine::GetCurCoroutine());
        ...
        if (!rt) {
            ...
            FISH_LOGDEBUG(EX_STRING_FUNC_WITH_ARG(fd, sys_fun_name) << ", Yield");
...
int nanosleep(const struct timespec *req, struct timespec *rem){
    ...
    auto co = fish::Coroutine::GetCurCoroutine();
    auto iom = fish::IoManager::GetCurIoManager();
...
int socket(int domain, int type, int protocol) {
    ...
    // 加入套接字管理器
    fish::FdManagerSgt::Instance().Get(fd, true);
...
int connect_with_timeout(int sockfd, const struct sockaddr *addr,
    ...
    auto ctx = fish::FdManagerSgt::Instance().Get(sockfd);
    if (!ctx || ctx->IsClosed()) {
        errno = EBADF;
        return -1;
    }
    ...
    if (timeout_ms != -1) {
        timer = iom->addConditionTimer(timeout_ms, [winfo, sockfd, iom]() {
            ...
            t->cancelled = ETIMEDOUT;
            iom->SetEvent(sockfd, EPOLLOUT, fish::Coroutine::GetCurCoroutine());
        }, winfo);
    }

    int rt = iom->SetEvent(sockfd, EPOLLOUT, fish::Coroutine::GetCurCoroutine());
    if (rt) {
        fish::Coroutine::Yield();
        if (timer) {
            timer->cancel();
...
int connect(int sockfd, const struct sockaddr *addr,
            socklen_t addrlen) {
    return connect_with_timeout(sockfd, addr, addrlen, fish::s_connect_timeout);
}
...
int accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
    ...
    if (accept_fd >= 0) {
        // 加入套接字管理器
        fish::FdManagerSgt::Instance().Get(accept_fd, true);
    }
    return accept_fd;
}
...
int close(int fd) {
    ...
    auto ctx = fish::FdManagerSgt::Instance().Get(fd);
    // 取消关于句柄的事件
    if (ctx) {
        auto iom = fish::IoManager::GetCurIoManager();
        if (iom) {
        ...
        fish::FdManagerSgt::Instance().Del(fd);
    }
    return close_f(fd);
}
