...
bool TcpServer::Init(Address::Ptr address, IoManager* io_manager, uint32_t client_handler_cnt) {
    if (is_init_) {
        return false;
    }
    if (!Bind({address})) {
...
bool TcpServer::Bind(const std::vector<Address::Ptr>& addrs/*, bool ssl*/) {
